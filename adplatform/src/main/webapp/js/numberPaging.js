...
function createGoToLink(currentPage, totalPage) {
    var goToHtml = '<li><span style="border-style: none;">GO</span></li>';
    goToHtml += '<li><input style="width:40px;height:19px;" id="specificPageNumber" type="text" value="' + currentPage + '"/></li>';
    goToHtml += '<li><a href="javascript:void(0);" onclick="gotoClick(' + totalPage + ')">Go</a></li>';
    return goToHtml;
}

function createPageInformation(currentPage, totalPage) {
    var pageInfoHtml = '<li><span style="border-style: none;">(' + currentPage + ' / ' + totalPage + ')</span></li>';
    return pageInfoHtml;
}
...
function getNumberPageLinks(startPage, currentPage, endPage, totalPage) {
    var pagingHtml = '';
    if (currentPage != 1) {
        pagingHtml += '<li><a href="javascript:void(0);" onclick="pagingClick(' + (currentPage - 1) + ',' + totalPage + ')">&lt</a></li>';
    } else {
        pagingHtml += '<li><a style="background:#BDBDBD;" href="javascript:void(0);">&lt</a></li>';
    }
    ...
    if (currentPage != endPage) {
        pagingHtml += '<li><a href="javascript:void(0);" onclick="pagingClick(' + (currentPage + 1) + ',' + totalPage + ')">&gt</a></li>';
    } else {
        pagingHtml += '<li><a style="background:#BDBDBD;" href="javascript:void(0);">&gt</a></li>';
    }
...
function createNumberPageLink(pageNum, currentPage, totalPage){
    var html;
    if (currentPage) {
        html = '<li class="cur"><a href="javascript:void(0);" onclick="pagingClick(' + pageNum + ',' + totalPage + ');">' + pageNum + '</a></li>';
    } else {
        html = '<li><a href="javascript:void(0);" onclick="pagingClick(' + pageNum + ',' + totalPage + ');">' + pageNum + '</a></li>';
    }
    return html;
}
